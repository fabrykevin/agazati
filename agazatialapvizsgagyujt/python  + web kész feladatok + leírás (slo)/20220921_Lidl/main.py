...
# ZARAS = 100
ZARAS = 16 * 60 * 60 # 16 órát van nyitva a bolt
MAX_VEVO_SZAM = 200
MAX_PENZTAR = 5
...
def allapotKiir(ido):
    ...
    # floor() - mindig lefelé kerkít floor(3.7) -> 3
    # ceil() - mindig felfelé kerekít ceil(3.1) -> 4
...
def penztarNyit():
    # ha nincs egy olyan péztár sem, ahol 3-nál kevesebben vannak, akkor nyissunk újat, ha lehet
...
def penztarbolKimegy():
    # minél többen vannak a pénztában, annál nagyobb valószínűséggel megy ki onnan vki
    for i in range(len(penztarak)):
...
def vevoPenztarbaMegy():
    ...
    # a pénztárba menteket (-1) törölni kell
    ujvevok = []
    for item in vevok:
        if item != -1:
            ujvevok.append(item)
